...
def test_make_unique_filename_if_exists():
    ...
    with (patch(target='src.scan_processor.pdf_merger.os.path.exists', autospec=True) as mock_exists,
          patch(target='src.scan_processor.pdf_merger.uuid.uuid4', autospec=True) as mock_uuid4):
